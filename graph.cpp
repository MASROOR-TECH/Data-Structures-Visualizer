...
public:
    ...
    std::string run_dijkstra(int startIndex) {
        ...
        bool* finalized = new bool[vertix];
    ...
    std::string run_prims(int startIndex) {
        ...
        while (!pq.empty()) {
            ...
            // If the extracted node is not the start node, it belongs to the MST
            if (parent[u] != -1) {
                total_mst_cost += current_key; // Add the cost (key) of the edge that brought it into the MST
            }
...
/**
 * Creates the final JSON output string.
 */
